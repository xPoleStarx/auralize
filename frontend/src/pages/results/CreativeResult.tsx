...
// JSON dosyasından yaratıcı quiz verilerini doğrudan alıyoruz
...
// Tip tanımlarını düzeltiyorum
interface AuraTypeDefinition {
  description: string;
  [key: string]: any;
}

// Au§§ra Data için bir arayüz tanımlayalım
interface AuraData {
  title: string;
  description: string;
  gradient: string;
...
// auraTypes nesnesini tanımlama
const auraTypeDefinitions: Record<string, AuraTypeDefinition> = {
  creative: { description: "Yaratıcı auran seni hayal gücü ve yenilik dünyasına bağlar." },
  analytical: { description: "Analitik auran mantıklı düşünme ve problem çözme yeteneklerini güçlendirir." },
  empathetic: { description: "Empatik auran duygusal zekanı ve başkalarıyla bağ kurma yeteneğini artırır." },
  visionary: { description: "Vizyoner auran geleceği görme ve büyük düşünme yeteneklerini destekler." },
  leadership: { description: "Liderlik auran ikna etme ve yönlendirme yeteneklerini güçlendirir." },
  balanced: { description: "Dengeli auran hayatın farklı yönleri arasında uyum sağlamanı destekler." },
  passionate: { description: "Tutkulu auran heyecan ve motivasyon enerjini yükseltir." },
  serene: { description: "Huzurlu auran sakinlik ve iç barış sağlar." },
  energetic: { description: "Enerjik auran hareket ve canlılık yaratır." },
  mindful: { description: "Farkındalıklı auran şu ana odaklanmana ve bilinçli yaşamana yardımcı olur." },
  career: { description: "Kariyer auran profesyonel hayatını ve yeteneklerini geliştirmene yardımcı olur." },
  mood: { description: "Duygu auran duygusal dengeyi ve ifade yeteneğini güçlendirir." },
  personal: { description: "Kişisel gelişim auran kendini tanıma ve geliştirme yolculuğunu destekler." }
};

// Insights interface'ini tanımlayalım
interface Insights {
  strengths: string;
  potential: string;
  thinkingStyle: string;
  auraTitle: string;
  source: string; // Burada spesifik string literal tipi yerine genel string kullanıyoruz
}

// CombinedData interface'ini ekleyelim
interface CombinedData {
  ...
  thinkingStyle: string;
  auraTitle: string;
  source: 'openai' | 'api' | 'default' | string; // Açıkça olası değerleri belirtiyoruz
}
...
const AuraResult: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setAuraData({
      ...
      description: auraTypeDefinitions[determinedType]?.description || 'Senin auran özel ve benzersiz bir enerji taşıyor.',
    ...
    const loadCombinedAuraData = async () => {
      ...
      try {
        ...
        // String karşılaştırma hatasını düzeltiyorum
        const sourceType = combinedData.source as string;
        let cacheSource: 'api' | 'default' = 'default';
        
        if (sourceType === 'openai' || sourceType === 'api') {
          cacheSource = 'api';
        }
        
        setApiCacheStat(cacheSource);
        ...
        // Hata durumunda yükleme durumlarını açık tut, boş yanıt geldiğinde loading state devam etsin
        // Böylece varsayılan metin yerine loading crystal görünecek
        setApiCacheStat('default');
        setIsApiReady(false); // Paylaşım butonu devre dışı kalsın
        
        // Yükleme durumlarını açık tut
        // setIsStoryLoading(false);
        // setIsFullStoryLoading(false);
        // setIsInsightsLoading(false);
      }
  ...
  // Hashtag işleme 
  const handleHashtagInputChange = (e: { target: { value: string } }) => {
                      ...
                      <div className="aura-story-text">
                        ...
                        {(() => {
                          ...
                          if (auraStory.includes("#### Aura Hikayesi") || 
                            ...
                            if (!insights.thinkingStyle) {
                              const thinkingMatch = auraStory.match(/#### Düşünme Tarzı.*?\: ([\s\S]*?)(?=####|$)/);
